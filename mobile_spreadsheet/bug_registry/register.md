...
| **BUG-05** | `g_namedRanges` Mutex & FFI Exception Barriers | `android/app/src/main/cpp/ffi_bridge.cpp` | Global `g_namedRanges` lacked thread safety under concurrent calls, and unhandled C++ exceptions in FFI exports could crash the Android process. | Introduced `g_namedRangesMutex` with `std::lock_guard` protection and added top-level `try-catch` barriers to FFI exports. | [BUG-05 Report](file:///d:/abuzer%20projects/Table%20sheets%20project/mobile_spreadsheet/bug_registry/bugs/bug_05_g_named_ranges_mutex/README.md) |
| **BUG-06** | AI Agent Column & Row Outline / Border Support | `lib/presentation/editor/widgets/grid_widget.dart` | Grid Painter ignored `cfStyle['border']`, C++ `js_engine.cpp` lacked `setOutline` and full `setBorder` parsing, and system prompt lacked border rules. | Added custom border rendering in `_RowPainter`, implemented `setOutline`/`setBorder` in C++, and updated AI system prompt. | [BUG-06 Report](file:///d:/abuzer%20projects/Table%20sheets%20project/mobile_spreadsheet/bug_registry/bugs/bug_06_ai_agent_column_row_outline/README.md) |
...
└── bugs/
    ...
    │   └── README.md
    └── bug_06_ai_agent_column_row_outline/
        └── README.md
```
